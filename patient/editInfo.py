...
class EditInfo:
    ...
    def create_widgets(self):
        ...
        self.emergency_contact_email_label.grid(row=9, column=0, padx=10, pady=10)
        self.emergency_contact_email_entry = tk.Entry(self.root)
        self.emergency_contact_email_entry.grid(row=9, column=1, padx=10, pady=10)
    ...
    def match_in_database(self, username, email, new_password, fname, lname, emergency_contact_email):
        ...
        if not user_info.empty:
            ...
            return (username != db_username or email != db_email or new_password != db_password or 
                    fname != db_fname or lname != db_lname or emergency_contact_email != db_emergency_contact_email)
        else:
            return False
    ...
    def update_info (self):
        ...
        y = user.iloc[0][8]
        # z = user.iloc[0][9] #what if i want to preserve disabled info in case its TRUE ? im getting error if i use it

        if  self.match_in_database(username, email, new_password, fname, lname, emergency_contact_email):
            ...
            newValues =(username, email, new_password if new_password else current_password, fname, lname, x, emergency_contact_email, y, False)
            try:
